  ...
  try {
    if (!id) {
      return {
        response: {
          role: 'assistant',
          content:
            'Chatbot is not connected to a valid domain yet. Please reload from the portal widget.',
        },
      }
    }
    ...
    if (chatBotDomain) {
      if (!openAiKey) {
        return {
          response: {
            role: 'assistant',
            content:
              'The assistant is not configured yet. Please add OPEN_AI_KEY to continue.',
          },
        }
      }
      ...
      if (customerEmail) {
        ...
        const currentCustomer = checkCustomer?.customer[0]
        const currentChatRoom = currentCustomer?.chatRoom[0]

        if (!currentCustomer || !currentChatRoom) {
          return {
            response: {
              role: 'assistant',
              content:
                'I could not find your active profile yet. Please share your email so I can continue.',
            },
          }
        }
        ...
        if (currentChatRoom.live) {
          ...
          await onRealTimeChat(currentChatRoom.id, message, 'user', author)
          ...
          messages: [
            {
              ...
              if the customer agrees to book an appointment send them this link ${appUrl}/portal/${id}/appointment/${
                currentCustomer.id
              }

              if the customer wants to buy a product redirect them to the payment page ${appUrl}/portal/${id}/payment/${
                currentCustomer.id
              }
          `,
            },
